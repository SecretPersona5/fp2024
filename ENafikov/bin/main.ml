...
let parse_command_line_args () =
  let rec process_args args current_config =
    ...
    | [] -> current_config
    | "-infer" :: remaining ->
      process_args remaining { current_config with enable_type_inference = true }
    | "-interpret" :: remaining ->
      process_args remaining { current_config with enable_interpreter = true }
    | "-file" :: filename :: remaining ->
      process_args remaining { current_config with input_file = Some filename }
    | arg :: remaining ->
      process_args remaining { current_config with direct_input = Some arg }
  in
  process_args
