...
(** SPDX-License-Identifier: LGPL-3.0-or-later *)
  ...
  Type inference error. Unknown name 'fac'.

  $ ../bin/main.exe -infer -file manytests/do_not_type/002if.ml
  Type inference error. TypeMismatch: int and bool.

  $ ../bin/main.exe -infer -file manytests/do_not_type/003occurs.ml
  Type inference error. CircularCheck failed. Type variable '1 occurs inside '1 -> '3.

  $ ../bin/main.exe -infer -file manytests/do_not_type/004let_poly.ml
  Type inference error. TypeMismatch: int and bool.

  $ ../bin/main.exe -infer -file manytests/do_not_type/015tuples.ml
  Type inference error. PatternError: Only variables are allowed on the left-hand side of let rec.

  $ ../bin/main.exe -infer -file manytests/do_not_type/016tuples_mismatch.ml
  Type inference error. TypeMismatch: ('0 * '1) and (int * int * int).

  $ ../bin/main.exe -infer -file manytests/do_not_type/097fun_vs_list.ml
  Type inference error. TypeMismatch: '2 list and '0 -> '0.

  $ ../bin/main.exe -infer -file manytests/do_not_type/097fun_vs_unit.ml
  Type inference error. TypeMismatch: unit and '0 -> '0.

  $ ../bin/main.exe -infer -file manytests/do_not_type/098rec_int.ml
  Type inference error. ExpressionError: Right-hand side of let rec must be a lambda expression.
